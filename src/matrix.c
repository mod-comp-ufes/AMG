...
void destroy_m (matrix *A) {
	free(A->val); free(A->diag);
	free(A->col_ind); free(A->row_ptr);
	free(A);
};
...
matrix * transpose (matrix *A) {
	...
	matrix *A_t;
	Node *aux;
	list **row_elem = (list **) malloc(n*sizeof(list *));
...
matrix * sum(matrix *A, matrix *A_t) {
	...
	Node *aux;
	int i, j, j_t, nnz=0;
	list **row_elem = (list **) malloc(A->n*sizeof(list *));
	...
	for (i=0; i<A->n; i++) destroy_l(row_elem[i]); free(row_elem);
	return A_s;
}
...
matrix * matmat_sparseproduct (matrix *A, matrix *B) {
	...
	matrix *C;
	Node *aux;
	list **row_elem = (list **) malloc(p*sizeof(list *));
	xb = (int *) calloc(r, sizeof(int));
	...
	for (i=0; i<p; i++) destroy_l(row_elem[i]); free(row_elem);
	return C;
};
