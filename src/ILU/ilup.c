...
{
	mat->n       = n;
	mat->nzcount = (int *) calloc(n,sizeof(int));
	mat->ja      = (int **) calloc(n,sizeof(int*));
	mat->ma      = (double **) calloc(n,sizeof(double*));
...
void SPARILU_setup (SparILU* lu, int n)
{
	...
	lu->L    = (SparMAT*) calloc(1,sizeof(SparMAT));
	SPARMAT_setup(lu->L, n);
	lu->U    = (SparMAT*) calloc(1,sizeof(SparMAT));
...
void CSRto_SPARMAT_setup (MAT* A, SparMAT* mat)
{
	...
	for (j = 0; j < n; ++j)
	{
		...
		if (len > 0) 
		{
			bja = calloc(len,sizeof(int));
			bra = calloc(len,sizeof(double));
			i   = 0;
...
int LEVEL_OF_FILL_lu (SparMAT* csmat, SparILU* lu, int p)
{
	...
	*------------------------------------------------------------------*/
	...
	levls = (int*)  calloc(n,sizeof(int));
	jbuf  = (int*)  calloc(n,sizeof(int)); 
	ulvl  = (int**) calloc(n,sizeof(int*));
	...
	for(i = 0; i < n; i++) 
	{
		...
		if(incl > 0 )
		{
			L->ja[i] = (int *) calloc(incl,sizeof(int));
		...
		if( k > 0 )
		{
			U->ja[i] = (int *) calloc(k,sizeof(int));
			memcpy(U->ja[i], jbuf+i, k*sizeof(int));
			/*-------------------- update matrix of levels */
			ulvl[i]  = (int *) calloc(k,sizeof(int)); 
